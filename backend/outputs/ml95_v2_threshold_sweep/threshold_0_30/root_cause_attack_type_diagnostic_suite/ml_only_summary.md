...
- Full ML label+type accuracy: **19/32** (59.38%)
- Precision / Recall / F1: **0.7241 / 1.0000 / 0.8400**
- FP / FN: **8 / 0**
- Expected attack distribution: `{'NONE': 11, 'IN_BAND': 10, 'SECOND_ORDER': 4, 'BLIND': 7}`
- ML attack distribution: `{'SECOND_ORDER': 6, 'BLIND': 7, 'IN_BAND': 16, 'NONE': 3}`
...
- `root_cause_attack_type_diagnostic_suite/java/023_SAFE_db_loaded_value_param.java` expected `SAFE / NONE` got `VULNERABLE / IN_BAND` risk `0.9504`
...
- `root_cause_attack_type_diagnostic_suite/php/030_SAFE_db_value_as_param.php` expected `SAFE / NONE` got `VULNERABLE / IN_BAND` risk `0.4907`
- `root_cause_attack_type_diagnostic_suite/php/031_BLIND_time_based_sleep.php` expected `VULNERABLE / BLIND` got `VULNERABLE / IN_BAND` risk `0.4907`
...
- `root_cause_attack_type_diagnostic_suite/python/008_SAFE_allowlist_decoy_stored_word.py` expected `SAFE / NONE` got `VULNERABLE / IN_BAND` risk `0.9381`
