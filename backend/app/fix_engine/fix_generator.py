...
This file renders the fix strategy selected by Model 2 into a concrete code example.
The model remains responsible for selecting A/B/C/D. The only automatic guard here
is a safety guard for SQL identifiers, because table names and ORDER BY columns
cannot be repaired with bound-parameter placeholders.
"""
...
def _line_with(pattern: str, code: str):
...
def _detect_order_by_injection(code: str) -> bool:
    """Detect a dynamic identifier directly in the ORDER BY position."""
...
def _detect_table_name_injection(code: str) -> bool:
    """Detect a dynamic table identifier directly after FROM/JOIN/UPDATE/INTO."""
    c = _strip_comments(code, "python")
    return _rx(
...
def _exec_receiver(code: str, language: str) -> str:
    if language == "python":
        m = re.search(r"\b([A-Za-z_]\w*)\s*\.\s*execute\s*\(", code)
        return m.group(1) if m else "cursor"
    m = re.search(r"\b([A-Za-z_$]\w*)\s*\.\s*(?:all|get|run|each|query|execute)\s*\(", code)
    return m.group(1) if m else "db"


def _template_variables(code: str) -> list[str]:
    """Return variables embedded in f-strings/template strings/braced expressions."""
    vars_: list[str] = []
...
def _concat_var(code: str) -> str:
    """Preserve the original variable used in SQL construction.

    This is a rendering helper, not a model decision rule. It keeps variable names
    like uid, sortColumn, tableName and PHP variables like $sort.
    """
    m = re.search(
        r"(?:SELECT|INSERT|UPDATE|DELETE|WHERE|ORDER\s+BY|FROM|JOIN|INTO)[^\n;]*[\"'`]\s*(?:\+|\.)\s*(\$?[A-Za-z_$]\w*)",
        code,
        re.I,
    )
    if m:
        return m.group(1)
    m = re.search(r"(?:\+|\.)\s*(\$?[A-Za-z_$]\w*)", code)
    if m:
        return m.group(1)
    vars_ = _template_variables(code)
    if vars_:
        return vars_[0]
    return "value"
...
def _sql_template_vars(code: str):
    # Robust Python f-string support. This handles inner quotes, for example:
    # f"SELECT * FROM users WHERE name = '{name}'"
    fm = re.search(r"\b[fF]([\"'])(.*?)(?<!\\)\1", code, re.S)
    if fm:
        template = fm.group(2)
        vars_ = re.findall(r"\{\s*([A-Za-z_]\w*)", template) or _template_variables(code) or ["value"]
        sql = re.sub(r"'?\{[^}]+\}'?", "?", template)
        return sql, vars_

    # JavaScript template string support.
    jm = re.search(r"`([^`]*(?:SELECT|INSERT|UPDATE|DELETE|WHERE|FROM|ORDER\s+BY)[^`]*)`", code, re.I | re.S)
    if jm:
        template = jm.group(1)
        vars_ = re.findall(r"\$\{\s*([A-Za-z_$]\w*)", template) or _template_variables(code) or ["value"]
        sql = re.sub(r"'?\$\{[^}]+\}'?", "?", template)
    ...
    parts = re.findall(r"[\"']([^\"']*(?:SELECT|INSERT|UPDATE|DELETE|WHERE|FROM|ORDER\s+BY)[^\"']*)[\"']", code, re.I)
    v = _concat_var(code)
    if parts:
...
def _python_exec_line(code: str, param: str):
    ...
    suffix = ""
    m = re.search(r"\.\s*execute\s*\([^)]*\)(\s*\.\s*(?:fetchone|fetchall|fetchmany)\s*\([^)]*\))", line)
    if m:
        suffix = re.sub(r"\s+", "", m.group(1))
    return indent, f"{prefix}{recv}.execute(query, ({param},)){suffix}"
...
def _generate_fix_A(code: str, language: str) -> str:
    ...
    qv = _query_var(code)
    param = vars_[0] if vars_ else "value"

    if not _has_execution_sink(code, language):
        return f"# Advisory fix: SQL is built but not executed here.\n{qv} = \"{sql}\"\nparams = ({param},)"

    if language == "python":
        indent, line = _python_exec_line(code, param)
        return f"{indent}# Fix: use parameterized query — never interpolate user input into SQL\n{indent}{qv} = \"{sql}\"\n{indent}{line}"
    if language == "javascript":
        return f"// Fix: use parameterized query\nconst {qv} = \"{sql}\";\nreturn {_exec_receiver(code, language)}.all({qv}, [{', '.join(vars_)}]);"
    if language == "php":
        return f"// Fix: use PDO prepared statements\n$stmt = $pdo->prepare(\"{sql}\");\n$stmt->execute([${param.lstrip('$')}]);"
    if language == "java":
        return f"// Fix: use PreparedStatement\nPreparedStatement stmt = conn.prepareStatement(\"{sql}\");\nstmt.setString(1, {param});\nResultSet rs = stmt.executeQuery();"
...
def _generate_fix_B(code: str, language: str) -> str:
    ...
    if language == "javascript":
        if table_mode:
            return f"// Fix: whitelist allowed table names — identifiers cannot be parameterized\nconst ALLOWED_TABLES = new Set(['users','orders','products']);\nif (!ALLOWED_TABLES.has({v})) throw new Error('Invalid table name');\nconst query = `SELECT * FROM ${{{v}}}`;\nreturn db.all(query);"
        return f"// Fix: whitelist allowed ORDER BY columns — identifiers cannot be parameterized\nconst ALLOWED_COLUMNS = new Set(['id','name','email','created_at']);\nif (!ALLOWED_COLUMNS.has({v})) throw new Error('Invalid sort column');\nconst query = `SELECT * FROM users ORDER BY ${{{v}}}`;\nreturn db.all(query);"

    if language == "php":
        var = v if v.startswith("$") else "$" + v
        if table_mode:
            return f'// Fix: whitelist allowed table names — identifiers cannot be parameterized\n$allowedTables = [\'users\',\'orders\',\'products\'];\nif (!in_array({var}, $allowedTables, true)) {{ throw new InvalidArgumentException(\'Invalid table name\'); }}\n$sql = "SELECT * FROM " . {var};\n$stmt = $pdo->query($sql);'
...
def _generate_fix_C(code: str, language: str) -> str:
    return "# Fix C: migrate complex raw SQL construction to a structured ORM/query builder."


def _generate_fix_D(code: str, language: str) -> str:
    return "# Fix D: do not execute SQL loaded from DB/config/user storage. Rebuild SQL from trusted static templates."
...
def generate_fix(
    ...
    # Safety guard: this does not replace Model 2's learning; it prevents invalid repairs.
    ...
    if "SECOND_ORDER_FLOW" in signals or model_fix == "D":
